...
import com.example.chen.wsscapp.View.SendFileView;
import com.example.chen.wsscapp.activity.DocumentFragment;
import com.example.chen.wsscapp.activity.SendFileActivity;
import com.example.chen.wsscapp.fragment.AudioFragment;
import com.example.chen.wsscapp.fragment.ImageFragment;
import com.example.chen.wsscapp.fragment.OtherFragment;
import com.example.chen.wsscapp.fragment.VideoFragment;
...
public class SendFileController implements View.OnClickListener, ViewPager.OnPageChangeListener,
    ...
    public SendFileController(SendFileActivity context, SendFileView view) {
        ...
        String targetId = mContext.getIntent().getStringExtra(MyApplication.TARGET_ID);
        String targetAppKey = mContext.getIntent().getStringExtra(MyApplication.TARGET_APP_KEY);
        long groupId = mContext.getIntent().getLongExtra(MyApplication.GROUP_ID, 0);
        if (groupId != 0) {
            ...
            if (controller != null) {
                switch (msg.what) {
                    case SEND_FILE:
                        ...
                        intent.putExtra(MyApplication.MsgIDs, controller.mMsgIds);
                        controller.mContext.setResult(MyApplication.RESULT_CODE_SEND_FILE, intent);
